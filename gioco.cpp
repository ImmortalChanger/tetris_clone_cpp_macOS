...
Gioco::Gioco() {
	...
	griglia = Griglia();
	blocchi = ottieniTuttiBlocchi();	// � una funzione per rigenerare un vettore con tutti i blocchi al suo interno
...
void Gioco::Disegna() {
	...
	switch(prossimoBlocco.id) {
		...
		case 3:
			prossimoBlocco.Disegna(415, 505);	// caso per centrare il blocco I
			break;

		case 4:
			prossimoBlocco.Disegna(415, 480);	// caso per centrare il blocco O
			break;

		default:
			prossimoBlocco.Disegna(440, 470);	// caso per centrare il resto dei blocchi
			break;
	}
...
void Gioco::muoviBloccoSotto() {
	...
	if (!gameOver) {
		...
		if (ilBloccoFuori() || bloccoAttaccato() == false) {
			bloccoCorrente.Movimento(-1, 0);
			bloccoBloccato();			// che bel gioco di parole, serve per non far pi� comandare il blocco dall'utente quando tocca il fondo
...
bool Gioco::bloccoAttaccato() {	// funzione per verificare se una cella � attaccata ad un'altra gi� occupata
