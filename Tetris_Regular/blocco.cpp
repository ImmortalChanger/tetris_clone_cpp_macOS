...
void Blocco::Disegna(int offsetx, int offsety) {
	...
	for (Posizione oggetto : soglie) {
		DrawRectangle((oggetto.colonna * grandezzaCella + offsetx) * prop, (oggetto.riga * grandezzaCella + offsety) * prop, (grandezzaCella - 1) * prop, (grandezzaCella - 1) * prop, colori[id]);
	} // usiamo gli offset per disegnare (in modo diverso) i blocchi successivi
}

void Blocco::Movimento(int rows, int columns) {

    (offsetRiga += rows) * prop;
    (offsetColonna += columns) * prop;
}
